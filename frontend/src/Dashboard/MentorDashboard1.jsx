...
const MentorDashboard = () => {
  ...
  useEffect(() => {
    ...
    const fetchSessions = async () => {
      try {
        ...
        console.log("Sessions response:", response.data); // Debugging
        ...
        console.error("Error fetching sessions:", err.response); // Debugging
      }
    };
  ...
  const handleAddSession = async () => {
    ...
    try {
      // Create date-time object with selected date and time
      ...
      console.log("Checking available sessions:", availableSessions);
      ...
      const isDuplicate = availableSessions.some(
        (session) =>
          ...
          Date.parse(session.date) === Date.parse(newSession.date) && // Fixed date comparison
      ...
      const response = await axios.post("http://localhost:5000/api/mentors/sessions", newSession);
      console.log("Session successfully created:", response.data);
  
      // Fetch updated session list after adding a new session
      const updatedSessions = await axios.get(
        `http://localhost:5000/api/mentors/sessions?mentorId=${encodeURIComponent(userId)}`
      );
      
      // const updatedSessions = await axios.get(`http://localhost:5000/api/mentors/sessions?mentorId=${userId}`);
      
      console.log("Updated sessions list:", updatedSessions.data);
  
      // Update session state
      setAvailableSessions(updatedSessions.data.filter((session) => session.status === "available"));
      setUpcomingSessions(updatedSessions.data.filter((session) => session.status === "booked"));
  
      // Reset form fields after successful session addition
      setDate(new Date());
      setTime("10:00");
      setDuration(60);
    } catch (error) {
      console.error("Error adding session:", error?.response?.data || error.message);
      setDuplicateError(error?.response?.data?.message || "An error occurred while adding the session.");
    }
  };
