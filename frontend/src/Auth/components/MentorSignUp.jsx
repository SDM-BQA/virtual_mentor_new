...
const onSubmit = async (data) => {
  try {
    ...
    const response = await axios.post("http://localhost:5000/api/mentors/mentorregister", formData, {
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="p-4 border rounded shadow">
        ...
        <div className="row">
          ...
          <div className="col-md-6 mb-3">
            ...
            <input type="text" className="form-control" {...register("mobile", { required: true, pattern: /^[0-9]{10}$/ })} />
            {errors.mobile && <small className="text-danger">Enter a valid 10-digit mobile number</small>}
          </div>
        </div>
        ...
        <div className="row">
          ...
          <div className="col-md-4 mb-3">
            ...
            {errors.city && <small className="text-danger">City is required</small>}
          </div>
        </div>
        ...
        <div className="row">
          <div className="col-md-6 mb-3">
            ...
            <select className="form-control" {...register("mentorIn", { required: true })}>
              <option value="">Select</option>
              <option value="Painting">Painting</option>
              <option value="Sketching">Sketching</option>
              <option value="Sculpture">Sculpture</option>
            </select>
            {errors.mentorIn && <small className="text-danger">Please select a category</small>}
          </div>
          <div className="col-md-6 mb-3">
            <label>Experience (Years)</label>
            <input type="number" className="form-control" {...register("experience", { required: true, min: 1 })} />
            {errors.experience && <small className="text-danger">Valid experience is required</small>}
          </div>
        </div>

        <div className="mb-3">
          <label>Awards/Achievements</label>
          <textarea className="form-control" rows="2" {...register("awards")}></textarea>
        </div>

        <div className="row">
          <div className="col-md-6 mb-3">
            <label>Social Media Links</label>
            <input type="text" className="form-control mb-2" placeholder="Instagram" {...register("instagram")} />
            <input type="text" className="form-control mb-2" placeholder="LinkedIn" {...register("linkedin")} />
            <input type="text" className="form-control mb-2" placeholder="Twitter" {...register("twitter")} />
            <input type="text" className="form-control" placeholder="Facebook" {...register("facebook")} />
          </div>
          <div className="col-md-6 mb-3">
            <label>Promotional Video Link</label>
            <input type="url" className="form-control" {...register("videoLink")} />
          </div>
        </div>

        <div className="mb-3">
          <label>Profile Picture</label>
          <input type="file" className="form-control" {...register("profilePicture")} />
        </div>

        <div className="mb-3">
          <label>Short Bio</label>
          <textarea className="form-control" rows="3" {...register("bio")}></textarea>
        </div>

        <div className="d-flex justify-content-between">
          <button type="submit" className="btn btn-success">Submit</button>
          <button type="button" className="btn btn-danger" onClick={() => reset()}>Clear Fields</button>
        </div>
      </form>
